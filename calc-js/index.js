...
const resulltInput = document.getElementById('result')
...
input.addEventListener('keydown', function(ev) {
    ...
    if (allowedKeys.includes(ev.key)) {
        // ev.key é a tecla relacionada ao evento
        input.value += ev.key
        return
    }
...
function calculate() {
    ...
    //Se funcionar o código segue para essa parte
...
document.getElementById('copyToClipboard').addEventListener('click', function(ev) {
    ...
    if (button.innerText === 'Copy') {
        ...
        window.navigator.clipboard.writeText(resulltInput.value)
    }
    else {
